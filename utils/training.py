    ...
    for k, test_loader in enumerate(dataset.test_loaders):
        ...
        for data in test_loader:
            ...
            if dataset.SETTING == 'class-il':
                mask_classes(outputs[0], dataset, k)
                _, pred = torch.max(outputs[0].data, 1)
...
def train(model: ContinualModel, dataset: ContinualDataset,
    ...
    for t in range(dataset.N_TASKS):
        ...
        # For each task, train SSL model for 80% of epochs and the rest fine-tune task specific model
        if args.train_ssl:
            model.net.to(model.device)
            model.net.train()
            desc = 'Training SSL for Task {}....'.format(t+1)
            ssl_epochs = math.floor(args.ssl_train_percentage * args.n_epochs)
            for epoch in tqdm(range(ssl_epochs), desc=desc):
                ssl_loss = 0
                ...
                if args.tensorboard:
                    tb_logger.log_ssl_loss(ssl_loss / len(train_loader), ssl_epochs, epoch, t)

            # reduce no.of epochs for supervised fine-tuning
            n_epochs = math.ceil((1 - args.ssl_train_percentage) * args.n_epochs)

            # freeze whole model except classification layer
            if args.freeze_backbone:
                layers_not_frozen = ['linear']  # , 'layer4', 'layer3', 'layer2'
                for name, param in model.net.named_parameters():
                    if not any(layer in name for layer in layers_not_frozen):
