...
class MNISTMLP(nn.Module):
    ...
    def __init__(self, input_size: int, output_size: int) -> None:
        ...
        self.ssl_linear = nn.Linear(100, 64)
        self.g = nn.Sequential(
                                nn.Linear(64, 64, bias=False),
                                nn.BatchNorm1d(64),  # comment out if batchsize is 1
        ...
        self.p = nn.Sequential(
            ...
            nn.BatchNorm1d(32),  # comment out if batchsize is 1
            nn.ReLU(inplace=True),
            nn.Linear(32, 64),
        )
        self.rot_linear = nn.Linear(100, 4)
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        out_rot = self.rot_linear(out)
        out_ssl = self.g(self.ssl_linear(out))
        out_pred = self.p(out_ssl)
        return  out_cls, out_rot, F.normalize(out_ssl), F.normalize(out_pred)
